...
try {
    ...
    if (!empty($searchTerm)) {
        $whereClause .= " AND (p.code LIKE ? OR p.brand LIKE ? OR p.model LIKE ? OR p.barcode LIKE ?)";
        $params[] = "%$searchTerm%";
    }
    ...
    if ($_POST) {
        if (isset($_POST['add_product'])) {
            ...
            if (empty($errors)) {
                // Insert new product
                $insertQuery = "INSERT INTO products (code, brand, model, category_id, supplier_id, branch_id, purchase_price, min_selling_price, stock, is_active, created_at) 
                                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, NOW())";
                $insertStmt = $db->prepare($insertQuery);
                $insertStmt->execute([$code, $brand, $model, $category_id, $supplier_id, $branch_id, $purchase_price, $min_selling_price, $stock]);
                
                $_SESSION['success_message'] = "Product added successfully! Product Code: $code";
        ...
        if (isset($_POST['edit_product'])) {
            ...
            // Update product
            $updateQuery = "UPDATE products SET code = ?, brand = ?, model = ?, category_id = ?, supplier_id = ?, branch_id = ?, purchase_price = ?, min_selling_price = ?, stock = ? WHERE id = ?";
            $updateStmt = $db->prepare($updateQuery);
            $updateStmt->execute([$code, $brand, $model, $category_id, $supplier_id, $branch_id, $purchase_price, $min_selling_price, $stock, $product_id]);
